...
#         bid_price_str = request.POST.get('bid_price')

#         try:
#             bid_price = float(bid_price_str)
#         except ValueError:
#             # Handle the case where the entered bid_price is not a valid float
#             messages.error(request, 'Invalid bid price. Please enter a valid number.')
#             return redirect('bid', task_id=task.pk)
...
#             messages.error(request, 'Bid price must be greater than or equal to task price.')
#             return redirect('bid', task_id=task.pk)
...
def bid(request, task_id):
    ...
    if request.method == 'POST':
        # Process the form submission if the request method is POST
        bid_price_str = request.POST.get('bid')

        if bid_price_str is None:
            # Handle the case where bid_price is not provided
...
def PaymentSuccesful(request, task_id):
    bid_price_str = request.POST.get('bid')
    return render(request, 'payment-success.html', {'bid_price_str': bid_price_str})
...
#     task = Task.objscts.get(id=task_id)
